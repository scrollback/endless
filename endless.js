...
(function (root, factory) {
	...
	return React.createClass({
		...
		lastState: { jumpToIndex: null, offset: 0 },
		...
		update: function () {
			...
			if(this.state.bottomReached && !this.state.bottomRemoved && viewBottom >= elBottom) {
				...
				below = Math.max(0, Math.max(
					...
					(viewBottom - itemEls[itemEls.length-1].offsetTop)/itemHeight // view far below rendered items
				) - above);
			}
		...
		componentDidUpdate: function (prevProps) {
			...
			var prevItems = prevProps.items.map(buildReactElement),
				...
				jumpRequired, newIndex = null, newMetrics, i,
...
//				console.log(topRemoved, topAdded, bottomRemoved, bottomAdded);
		...
		render: function () {
			return buildReactElement(['div', {style: {position: 'relative'}},
				['div', {style: {
					height: this.state.topRemoved + (this.state.topReached? 0: this.props.margin)
				}}],
				['div', {ref: 'items'}].concat(this.props.items),
				['div', {style: {
